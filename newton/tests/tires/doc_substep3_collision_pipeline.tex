...
Chrono uses the world vertical direction (Chrono: \texttt{ChWorldFrame::Vertical()}).
In the default ISO/Z-up convention this is \(\mathbf{e}_z = (0,0,1)\).

\paragraph{Shared primitives}
All three modes use the following recurring computations (with Chrono names in parentheses).
These are geometric helpers; no forces are computed here.
\begin{itemize}
  \item \textbf{Wheel forward direction} (\texttt{wheel\_forward}):
    ...
    \]
    This is the wheel ``rolling'' direction on flat ground (undefined if \(\mathbf{n}_d\parallel \mathbf{e}_z\)).
  \item \textbf{A point near the ``bottom'' of the disc in the world-vertical direction}
    ...
    \]
    This is the rim point which is lowest in the \(-\mathbf{e}_z\) direction (i.e.\ \(-\mathbf{e}_z\) projected into the disc plane).
...
High-level idea: sample the terrain once, treat the terrain locally as a plane (using the sampled normal), then compute
an approximate disc--plane intersection point and a corresponding penetration depth.
This is a first-order approximation (most accurate on locally planar/constant-slope terrain).

\begin{enumerate}
  \item Compute \(\mathbf{p}_b\) and query terrain properties (height \(h_c\), normal \(\mathbf{n}\), friction \(\mu\)) at
    \(\mathbf{p}_b + 2R\,\mathbf{e}_z\).
    (Chrono does \emph{not} re-query \(\mu\) at the final contact point in this mode; it uses this sampled value.)
  \item Early reject if the disc center is at/below the sampled terrain height:
    \[
      \mathbf{c}\cdot\mathbf{e}_z \le h_c \;\Rightarrow\; \text{no contact}.
    \]
  \item Compute a preliminary penetration estimate using the terrain height under \(\mathbf{p}_b\):
    \[
      d_0 = (h_c - \mathbf{p}_b\cdot\mathbf{e}_z)\,(\mathbf{e}_z\cdot\mathbf{n}).
    \]
    Here, \((h_c - \mathbf{p}_b\!\cdot\!\mathbf{e}_z)\) is the \emph{vertical} gap, while \((\mathbf{e}_z\!\cdot\!\mathbf{n})\) projects it
    onto the sampled terrain normal direction.
  \item Compute the slope-aware rolling direction (Chrono later calls this \texttt{longitudinal}):
    \[
      \mathbf{x} = \mathbf{n}_d \times \mathbf{n}.
    \]
    If \(\lVert \mathbf{x}\rVert^2 < 10^{-3}\), Chrono treats the disc as (almost) horizontal and returns no contact.
  \item Normalize \(\mathbf{x}\) and correct the depth:
    \[
      d = R - (R - d_0)\,(\mathbf{f}\cdot \hat{\mathbf{x}}).
    \]
    If \(d \le 0\), there is no contact.
    On flat terrain, \(\hat{\mathbf{x}}=\mathbf{f}\) and this reduces to \(d=d_0\).
  \item Compute the contact point on the disc:
    \[
      \mathbf{p}_c = \mathbf{c} + (R-d)\,(\mathbf{n}_d \times \hat{\mathbf{x}}).
    \]
  \item Construct the contact frame axes (Chrono returns these via a quaternion round-trip):
    \[
      \mathbf{x}=\hat{\mathbf{x}},\quad
...
\begin{verbatim}
f = normalize(cross(nd, ez))
pb = c + R * cross(nd, f)
(hc, n, mu) = terrain.GetProperties(pb + 2R*ez)
if height(c) <= hc: return no_contact
d0 = (hc - height(pb)) * dot(ez, n)
x = cross(nd, n); if |x|^2 < 1e-3: return no_contact
x = normalize(x)
d = R - (R - d0) * dot(f, x); if d <= 0: return no_contact
pc = c + (R - d) * cross(nd, x)
frame = (x=x, y=cross(n,x), z=n)
\end{verbatim}
